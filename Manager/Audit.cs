...
namespace Manager
{
    public class Audit : IDisposable
    {
        ...
        public static void GenerationCertificationSuccess(string username)
        {
            if (customLog != null)
            {
                string GenerationCertificationSuccess = AuditEvents.GenerationCertificationSuccess;

                string message = String.Format(GenerationCertificationSuccess, username);
            ...
            else
            {
                throw new ArgumentException(
                    string.Format(
                        "Error while trying to write event (eventid = {0}) to event log.",
        ...
        public static void RevocationCertSuccess(string username)
        {
            if (customLog != null)
            {
                string RevocationCertSuccess = AuditEvents.RevocationCertSuccess;
                string message = String.Format(RevocationCertSuccess, username);
        ...
        public static void ConnectionSuccess(string username1, string username2)
        {
            if (customLog != null)
            {
                string ConnectionSuccess = AuditEvents.ConnectionSuccess;
        ...
        public static void ConnectionFailed()
        {
            if (customLog != null)
            {
                string ConnectionFailed = AuditEvents.ConnectionFailed;
